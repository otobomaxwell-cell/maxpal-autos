...
export function PriorityUrgency() {
  return (
    <section id="priority" className="bg-bg text-ink pt-14 pb-0 sm:pt-24">
      <div className="mx-auto max-w-295 px-6 pb-14 sm:pb-16 border-t-4 border-brand">
        <p className="font-mono text-[0.8125rem] tracking-[0.14em] uppercase text-brand-text font-semibold mt-8 mb-3.5">
      ...
      <div className="h-3 bg-[repeating-linear-gradient(135deg,#1a0000_0_16px,#ff242b_16px_32px)]" />

      <div className="bg-dark text-dark-ink py-14 sm:py-20">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3.5">
            {priorityIdealFor.map((item) => (
              <li key={item} className="flex items-start gap-3 font-semibold">
                <TriangleIcon className="w-[18px] h-[18px] mt-0.5 flex-none text-brand" />
                {item}
              </li>
            ))}
          </ul>

          <p className="mt-8 font-mono text-[0.9375rem] font-semibold text-brand">
            Priority appointments carry an additional priority charge.
          </p>

          <div className="mt-8">
            <CtaLink href="#appointment" variant="primary">
